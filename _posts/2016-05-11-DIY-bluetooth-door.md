...
要让买来的蓝牙模块按照我们的意愿去工作、收发数据，要先对模块进行配置。配置包括设备名称、连接密码、通讯波特率……不过用担心，之前我已经专门写了一篇关于蓝牙模块配置的文章，所以这里就不啰嗦了，请戳——>[HC-05蓝牙模块的参数设置与使用-蓝牙门锁开发笔记]({% post_url 2016-05-11-usage-of-bluetooth-model %})
...
当初急急忙忙把展览模型做出来，然后就一直不断改进，加上DS1302实时时钟、忘关门提醒及防盗报警功能，现在舍友正在编写我们自己安卓APP控制软件（我们IT文化节APP大赛的作品），看来功能基本定型了，是时候汇报一下了。
