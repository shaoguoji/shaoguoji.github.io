...
与许多可穿戴设备的心率传感器相比还， PulseSensor 还存在很大差距，而自己写程序也仅仅是达到「勉强可用」的程度，输出数据偶尔还是会有大波动。代码也还有许多可改进的地方（比如将 20ms 的数据采样处理用定时器中断实现）。突然觉得**传感器采集到数据只是前提，对数据的处理才是一切应用的核心**，不断地调整参数、改良算法也是整个过程中最有趣的部分。
