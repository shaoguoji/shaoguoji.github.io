...
> 一旦掌握了基于接口的设计方法，就能够在服务于众多应用程序的通用接口基础上建立应用程序，从而加快开发速度。在一些 C++ 环境中的基础类库就体现了这种效果。增加对现有软件（接口实现库）的重用，能够降低初始开发成本，同时还能降低维护成本，因为应用程序的更多部分都建立在通用接口的实现之上，而这些实现无不经过了良好的测试。【P IV】
...
> 书中提供的一些接口是针对数据结构的，但本书不是介绍数据结构的，本书的侧重点在算法工程（包装数据结构以供程序使用），而不再数据结构算法本身。然而，接口设计的好坏总是取决于数据结构与算法是否合适，因此，本书可算是传统数据结构和算法教材（如 Robert sedgewick 所著的 Algorithms in C）的有益补充。【P V】
...
书中原话是「在大学介绍性的编程课程中了解了 C 语言」，这话放到国内基本没法玩（国外的「介绍性的编程课程」这么吊的么……），只限于大学那点 C 语言皮毛要啃这书有点难度。至少要把指针玩熟了，看到代码中的下标、星号、指针偏移能在脑子里放映「内存动画片」就差不多了。
...
1. 文学程序是一种不受限于特定语法，而是为文字表述服务的一种代码表达方式
...
说白了文学编程就是写书用的，代码要为作者的表述服务，其表现在代码的出场次序上，「代码块可以按最适于理解的顺序给出」，所以不必遵循编程语言强加的顺序（如 C 语言的「先声明后使用」）。
