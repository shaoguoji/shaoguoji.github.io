...
注意到 `scanf()` 读入数据时会**跳过必要的空白符（如空格、换行、制表符等）**，准确来说是在读取非字符型数据时才跳过。也就是说在多个输入数据之间本来就可以顺便加空白字符，对于下面的常见代码：
...
### 小结
 ...
 大多数人都懂得 `scanf()` 函数的常规用法，但若是遇到「非主流」的冷门奇葩情况时，就要求我们对 `scanf()` 函数的执行流程以及输入缓冲区有一定的了解，才能解释一些诡异的现象。事实上 `scanf()` 函数并不常用，甚至还有的程序员建议干脆完全避免使用它，但作为学习，我们多了解一点也无妨，最后附上《C Primer Plus》中对 `scanf()` 说明的一段话。
