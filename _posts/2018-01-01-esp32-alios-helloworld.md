...
在实时操作系统大家族中，常见的 µC/OS-III、FreeRTOS 等 RTOS 严格意义上只能算一个 kernel（只包含 OS 基本服务）。随着物联网时代到来，出现了 AliOS Things、RT-Thread 这些「时髦」的操作系统，大佬们在实时内核的基础上增加了大量组件，囊括通信协议栈、低功耗管理、安全加密算法、FOTA（远程固件升级）等功能，可以说目的十分明确 —— 直奔物联网。
...
早在 2016 年乐鑫 ESP32 和阿里云物联网系统 YoC 已经有了合作。去年 10 月份的云栖大会上阿里提出了 AliOS Things，不久之后项目开源并支持了 ESP32，同时为开发者提供了许多开发工具。如何把 AliOS Things 编译烧写到 ESP32 DevKitC 
...
### 安装 VS Code、alios-studio 扩展

项目 Wiki 已经有详细软件安装说明文档，按照步骤把 VS Code 和 alios-studio 扩展装好即可：

![图4 安装 VS Code 与 alios-studio](http://odaps2f9v.bkt.clouddn.com/18-1-2/56288242.jpg)

---

### 获取 AliOS Things SDK 和 ESP32 工具链
...
同样的解压到 `D:\msys32` 目录下，SDK 和 Toolchain 用于后续配置 VS Code 开发环境。

---

### 配置 SDK path 与 Toolchain path
...
#### 修改 alios-studio Toolchain 判断规则（后期项目完善后可跳过）

**此步骤非必须，由于我目前使用的 v1.1.2 版本的 SDK 尚未完善**，多少会存在一些小 bug。比如在添加 Toolchain path 的合法性判断，目前只实现了 arm 的，所以对 ESP32 的 xtensa 工具链会误报非法路径。
...
新建工程需要把 sdk 赋值到工程文件夹，比较耗时，请耐心等待。完成后 VS Code 会重新打开一个文件夹视图窗口，表示一个 alios-studio 工程，在左侧的目录中打开 `helloword.c` 文件，当中的 `application_start` 函数是应用程序的入口。helloworld 程序的运行结果是在串口以 5 s 的间隔打印 "" 调试字符串。

![图8 工程代码](http://odaps2f9v.bkt.clouddn.com/18-1-2/7029919.jpg)

视频演示：[AliOS Things Tutorial: 1 Hello World 应用](http://v.youku.com/v_show/id_XMzI2MTYyNDAwOA)
...
编译生成的文件在工程的 out 目录下，其中`out\helloworld@esp32devkitc\binary\helloworld@esp32devkitc.bin` 是要烧到板子上的固件。
...
引导程序和分区表的 bin 文件在 SDK 目录 `D:\AliOS-Things-master\platform\mcu\esp32\bsp` 下，用户程序 bin 由 alios-studio 编译得到，上面有说明。3 个 bin 文件烧录地址如下：
...
**系统启动时从 0x1000 地址处开始执行，引导程序会读取分区表确定内存分布，然后执行用户程序代码。因此全部 bin 都要烧到正确的地址才能正常执行程序，这一点需要特别注意。**
...
*其中 `bootloader.bin` 和 `custom_partitions.bin` 从 SDK 目录获取，用户 bin 通过子目录下搜索 "@esp32devkitc.bin" 结尾的文件得到。*

---

### 运行结果

点击 VS Code 下方的 Connect Device 按钮（选择 COM 号，波特率 115200），通过在 alios-studio 自带工具连接开发板串口（或其他串口工具）连接开发板，如果收到 ESP32 每隔 5 s 发过来的调试信息，说明 helloworld 运行成功！
